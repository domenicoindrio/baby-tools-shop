...
To quickly get hands-on with this project, make sure that [Docker is installed](https://docs.docker.com/desktop/), then navigate to the folder containing the `Dockerfile` and run following commands:
...
#      that contains all the files needed for the docker image
...
```
At this point the container is running and the Django app within is accessible by navigating to [`http://localhost:8025`](http://localhost:8025) or [`http://127.0.0.1:8025`](http://127.0.0.1:8025). For more in-depth information, check the [Usage](#usage) section.
...
This section will cover some useful tips when trying to interact with this repository:

- Settings & Configuration for Django can be found in [`babyshop_app/babyshop/settings.py`](./babyshop_app/babyshop/settings.py)
- Routing information, such as available routes, can be found under any `urls.py` file in `babyshop_app` and corresponding subdirectories
...
- Environment variables can be put in a `.env` file and can be referred to it while running the container, for ex.:
    ...
    ```
- For local testing with Django, before creating a container, a python `venv` proves useful (optionally [`pyenv`](https://github.com/pyenv/pyenv?tab=readme-ov-file#installation) for managing multiple Python versions)
...
> [!CAUTION]
> If you're running this on a remote server, there are more tweaks to apply, especially what concerns [security](#additional-information) and sensible data.  
> For quick testing purposes you can:
> - either add (before building the image) your server's IP address to `ALLOWED_HOSTS` in `settings.py` (to avoid `Invalid HTTP_HOST header` errors) 
> - or manage it with environment variables.
>
> Afterwards you should be able to visit the app at `http://<server_ip_address>:8025`.

## Docker useful commands

Here are a couple of useful Docker terminal commands:
```bash
$ docker images
...
- For more detailed information about configurations, security, and settings for Django, please refer to the official [Django documentation](https://docs.djangoproject.com/en/5.2/).
